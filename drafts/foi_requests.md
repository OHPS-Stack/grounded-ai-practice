...
specification and the existing AI Skills Hub FOI threads were read
first, and the request rebuilt on what they showed (`research_log.md`
Entry 074). Grounded in `research_log.md` Entries 044, 051, 053, 056,
060, 061 and 074, each read directly at drafting per the
re-read-never-recall rule. Sending, the route, and the requester
details are per-item creator decisions; `[NAME]` and contact details
are supplied at send time, never stored here.

**Precedent findings, 2026-08-12** (detail in Entry 074). The
disclosed specification (FOI2026/00071) supplies the contractual
frame: Annex A mandatory metrics, quarterly reporting with six dated
review meetings, six-monthly white papers disseminated to a
stakeholder list, engagement expectations of 500 then 2,000
organisations, and an initial period to March 2026 with a 12-month
option decided at its end. FOI2026/00204 asked for the quarterly
reports wholesale and was **refused in full** under sections 41 and
43(2); FOI2026/00458 asked for measures and performance and was
**answered**, disclosing a skills white paper and a KPI breakdown as
annexes. Request 2 is therefore built on the narrow-ask pattern —
metric values and disseminated documents, never "the reports". DSIT's
"information not held" on Hub build costs places Hub operational
records with UKRI.

**Addendum, same day:** the FOI2026/00458 annexes and both response
letters were then read directly (`research_log.md` Entry 075). The
disclosed KPI set contains eighteen process and delivery indicators
and no user, learner or outcome metric; the unique-visitors question
was answered by reference to a document containing no visitor data;
demographic user-research information "was not recorded"; WCAG 2.2 AA
compliance runs "from March 2026", ten months after the May 2025 MVP.
Items 5 and 8 were sharpened accordingly: the aggregated Annex A
survey results joined item 5 (KPI 11 records over 650 responses
held), and item 8 now asks for the sector, region and size breakdowns
the KPI definitions themselves specify. **Second addendum, same day:** the 118-page white paper was then read
in full before dispatch, at the creator's direction
(`research_log.md` Entry 076). Three consequences. Item 6 now names
the second White Paper by reference to the first's own page-79
commitment, which is far harder to answer evasively than a generic
ask. A new item 6a requests the survey questionnaire and per-question
bases, made specific by the four inconsistent sample sizes and the
derived 96 per cent statistic the paper itself documents. And PwC's
pre-launch audience research is **not** requested at all: it is
published inside this white paper, so asking for it would invite a
section 21 refusal (information reasonably accessible by other
means) and weaken the rest of the request.

**Deliberately not asked**, and why:

- Per-partner completion figures as a primary item. DSIT's explainer
  pre-flags "commercial sensitivity", so a per-partner league table
  invites a section 43 refusal of the whole request. The counting
  rules are asked for instead, with partner-identifying details
  explicitly severable.

- Internal advice on the December 2025–January 2026 target change
  (7.5m to 10m). Policy advice invites section 35; the recorded
  measurement basis is asked for instead, which is methodology.
...
## Request 2 — UK Research and Innovation (for Innovate UK)

**To:** foi@ukri.org (or via WhatDoTheyKnow's UKRI page)
...
- **WhatDoTheyKnow:** the request, the requester's real name and
  every response are published permanently and automatically. That
  creates a citable public record (a future post or the report can
  link the live thread), fits the project's openness, and the
  platform chases deadlines. The cost is the name–project association
  being public from day one.

- **Direct email:** private; responses arrive to the chosen mailbox
  (the project's domain mailbox fits) and can be published later by
