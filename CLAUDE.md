...
- `docs/` — the public landing site, served by GitHub Pages from `/docs`
  ...
  static site: no framework, no JavaScript, nothing loaded from a third
  party. **Everything in `docs/` ending `.html` is generated output —
  edit `site/` instead**, since a hand edit here is lost at the next
  build. Its data figures are generated by
