...
| `[LLE-GOV26]` | Government/Official (DfE/Student Finance England) | Lifelong Learning Entitlement guidance for providers and the LLE campaign site, gov.uk — **SEARCH-LEVEL ONLY, not fetched directly.** Figures below are from a search synthesis of gov.uk, the House of Commons Library briefing (CBP-9756), the Office for Students modular-provision page and HESA. Needs a primary read before any claim is used. |
| `[APPRENTICE-AUG26]` | Mixed — Government/Official policy reported largely through Vendor/Commercial training providers | August 2026 apprenticeship funding reforms and the new short "apprenticeship units". **SEARCH-LEVEL ONLY, not fetched directly**, and the reachable sources were training providers (NSAR, VQ Solutions, NowSkills, Chesterfield College) with a commercial interest in apprenticeship uptake. The underlying gov.uk funding rules exist and were not read. Treat every figure as unverified. |
...
| `[MOG-JUL26]` | Government/Official (a statement of the government's own decisions, made on the Prime Minister's behalf — authoritative for what changed, silent on where individual programmes landed) | *Machinery of Government Changes*, written ministerial statement HLWS298, made by Baroness Smith of Basildon on behalf of the Prime Minister, 21 July 2026 — PRIMARY, read directly via Parliament's written-statements service. Abolishes DSIT and redistributes its functions to the Cabinet Office, DCMS and the renamed Department for Business, Innovation, Science and Trade. Corroborated by the replacement notice on DSIT's own gov.uk page. |
...
| `[GPU-LAUNCH-PRICES26]` | Vendor/Commercial (vendor launch announcements reported via tech press; MSRP is a stated intention, not a transaction) | Launch/list prices gathered 2026-08-12 for the budget-VRAM comparison. **Vendor UK MSRPs, in sterling inc. VAT:** Intel Arc B580 £249.99 at UK launch 2024-12-13 (search synthesis); RTX 5070 £539 (KitGuru, 2025-03-05, "launching at an MSRP price point of £539" — read directly; Nvidia later listed £529); RTX 5060 Ti 16GB £399 and 8GB £349 (KitGuru, 2025-04-16 — read directly); RTX 5090 £1,919 (carried from `[GPU-PRICES-UK26]`); RX 9060 XT 16GB £314.99 (search-level only — two direct fetch routes returned 403). **US list, no UK RRP published:** Arc Pro B50 $349 at launch Sept 2025, revised up from a $299 Computex 2025 target, reported as memory-cost and tariff driven (multi-outlet search synthesis: TweakTown, Guru3D, TechPowerUp); Arc Pro B60 — **Intel published no MSRP**, indicating ~$500 at Computex; $599 was the launch retail entry (ASRock via Central Computers) and Sparkle listed $799 (VideoCardz/TechPowerUp, search synthesis); Arc Pro B70 $949 (corroborates `[SR-B70-26]`); Radeon AI PRO R9700 $1,299, launched 2025-10-27, partner cards $1,244–$1,329 (multi-outlet search synthesis). **Conversion used in the figure:** $1 = £0.7396 (GBP/USD 1.3521, 2026-08-12, search-level) plus 20% VAT. |
| `[AA-INDEX26]` | Vendor/Commercial (AI benchmarking company; the public leaderboard is marketing for its premium data products) | Artificial Analysis, Intelligence Index leaderboard and model pages — read directly 2026-08-12. Cross-check scale only: the index is rescaled between versions (the same Qwen3.6 27B quoted as 46 in third-party citations and 38 on the current page), so scores compare within one snapshot, not across time. Snapshot 2026-08-12: Claude Opus 5 (max) 63, Claude Fable 5 62, GPT-5.6 Sol (max) 61, Kimi K3 (max) 60; small open-weights class (134 models): Qwen3.6 27B 38 (#1), Qwen3.6 35B A3B 32, Gemma 4 31B 30, gpt-oss-20b (high) 15. artificialanalysis.ai/leaderboards/models |
| `[EPOCH-ECIDATA26]` | Independent/Academic-adjacent (nonprofit AI-trends research organisation; methodology openly published; CC-BY) | Epoch AI Capabilities Index dataset — PRIMARY, both files retrieved directly 2026-08-12 from the site's own data endpoints: eci_scores.csv (222 aggregated models: ECI, 95% CI, release date, open/closed accessibility) and benchmarked_models.csv (per-version rows). A composite of 50+ benchmarks; the scale carries two calibration anchors listed without CIs (Claude 3.5 Sonnet = 130.0, GPT-5 = 150.0). epoch.ai/data/eci_scores.csv, via epoch.ai/eci |
| `[EPOCH-GPUGAP25]` | Independent/Academic-adjacent (nonprofit; CC-BY) | Epoch AI, "Frontier AI performance becomes accessible on consumer hardware within a year", data insight, 2025-08-15, data updated 2025-11-18 — PRIMARY, read directly 2026-08-12. Open models runnable on one consumer GPU (RTX 5090 assumption: ~29.8 GiB usable, so ≤~40B parameters at 4-bit with an 8K context) match frontier performance after a 6–12 month lag (GPQA-Diamond 7.4 months, MMLU-Pro 7.3, LM Arena Elo 12.4). Stated caveat: small models are likelier benchmark-optimised, so the real-world lag "may be somewhat longer". A year old at reading; no refreshed successor found. epoch.ai/data-insights/consumer-gpu-model-gap |
| `[LLMEXPLORER26]` | Vendor/Commercial (model directory operated by extractum.io; popularity-weighted "Score") | llm-explorer.com, "LLMs for 32GB VRAM" list — read directly 2026-08-12, a creator-suggested lead. Indexes 58,133 HuggingFace repositories; useful as a file directory, not as a ranking: its VRAM column assumes full 16-bit weights (Llama 3 8B listed at 16.1 GB), so the 32 GB filter excludes the quantised 27–36B models that fit in practice, and its default sort is popularity-weighted, surfacing 2024-era models. Logged for the defects as much as the content. llm-explorer.com/list/?32GB |
| `[OPENAI-4OMINI24]` | Vendor/Commercial (first-party product announcement) | OpenAI, "GPT-4o mini: advancing cost-efficient intelligence", 2024-07-18 — PRIMARY, read directly 2026-08-12: "In ChatGPT, Free, Plus and Team users will be able to access GPT-4o mini starting today, in place of GPT-3.5." Anchors the free-tier product history used in the capability figure. openai.com/index/gpt-4o-mini-advancing-cost-efficient-intelligence/ |
| `[INTEL-XPUM26]` | Vendor/Commercial (first-party open-source tooling, documenting its own hardware) | Intel XPU Manager repository — read directly 2026-08-13. Free, open-source monitoring and management for Intel data-centre GPUs, with `xpu-smi` as the local command-line interface; the supported-devices list names the **Intel Arc Pro Series GPU**. Reports power usage against cap in watts, plus GPU utilisation, temperature, frequency, memory usage and throughput, fabric health, and volatile uncorrectable ECC counts. Linux (Ubuntu 24.04/26.04 listed) and Windows Server with limited features. Corrects the conclusion drawn in Entry 071 that GPU power is unreachable in software. github.com/intel/xpumanager |
...
| `[INTEL-DGPU26]` | Vendor/Commercial (Intel first-party developer documentation) | Intel dgpu-docs pages, all PRIMARY, read directly (fetch summary) 2026-08-14: the OMIX installation guide (installing-omix.html — Intel Open Middleware Xe for Arc Pro B50/B60/B65/B70, on Ubuntu Desktop 26.04, 24.04.4, or 24.04 with the 6.17 HWE kernel; clean-system install of a pinned, validated component set); the OMIX support matrix (releases 0.1.0/0.2.0/0.3.0, B70 validated in all three, Ubuntu 26.04 the listed host OS throughout, 24.04 appearing as a container OS); the Xe KMD supported-hardware table (B70 = device E223, Xe2/Battlemage, full support Ubuntu 25.10+ / kernel 6.17; the A770 absent — Alchemist remains on i915); the intel-graphics PPA client guide (ppa:kobuk-team/intel-graphics for Ubuntu 26.04 and 24.04; "Ubuntu 26.04 and later provides native support for Lunar Lake, Battlemage, and Panther Lake"; compute packages libze-intel-gpu1, libze1, intel-metrics-discovery, intel-opencl-icd, clinfo, intel-gsc); PyTorch 2.11.0 container release notes (B70 supported, validated on an Ubuntu 24.04.4 host, OMIX-based, "not intended for production use", bundled oneCCL "does not include full optimization for PCIe collective communications"); and vLLM 0.21.0 container release notes (B70 validated, Ubuntu 24.04.4 host, "not intended for production use", dependency CVE-2025-69872 noted). The dgpu-docs landing page itself returned an empty body to the fetcher; deep pages were read without their index. dgpu-docs.intel.com |
| `[IPEX-EOL26]` | Vendor/Commercial (Intel first-party) | Intel Extension for PyTorch installation page — PRIMARY, read directly 2026-08-14: "Intel® Extension for PyTorch will reach end of life (EOL) by the end of March 2026", recommending native PyTorch, into which Intel CPU and GPU support has been upstreamed. Distinct from the IPEX-LLM archival already logged as `[IPEXLLM-GH26]` — two different projects. pytorch-extension.intel.com/installation |
| `[PYTORCH-XPU26]` | Vendor/Commercial (PyTorch project documentation; the Intel-GPU support it documents is Intel-contributed) | PyTorch, "Getting Started on Intel GPU" (2.13 documentation; the stable-notes URL redirects here) — PRIMARY, read directly 2026-08-14. Client GPU support from PyTorch 2.5: Arc A-Series (Alchemist) and Arc B-Series (Battlemage), validated on Windows 11 and Ubuntu 24.04/25.10 — Ubuntu 26.04 not in the validated list at reading. docs.pytorch.org/docs/2.13/notes/get_start_xpu.html |
| `[OLLAMA-DOCS26]` | Vendor/Commercial (first-party product documentation) | Ollama hardware-support and Docker pages — PRIMARY, both read directly 2026-08-14: additional GPU support on Windows and Linux via Vulkan; Vulkan bundled in the ollama/ollama Docker image and "enabled by default when the container can access the GPU devices"; OLLAMA_VULKAN=0 disables it; GGML_VK_VISIBLE_DEVICES selects devices. Neither page carried an "Experimental" label at reading. docs.ollama.com/gpu; docs.ollama.com/docker |
| `[ARC-PLATFORM26]` | Vendor/Commercial (Intel support articles and IPEX documentation, reached at **search level** — intel.com returned 403 to two direct fetch attempts) | Arc host-platform requirements, gathered 2026-08-14. Resizable BAR stated as required for intended performance on Arc Pro A- and B-series, with Above 4G Decoding its BIOS precondition; Intel's platform list is Intel-centric but states that other platforms with Resizable BAR or AMD Smart Access Memory enabled may also support these cards, with Ryzen 3000 (excluding 3000G), 5000 and 7000 named in the surfaced coverage. Separately, IPEX documentation states WSL2 support is experimental and that **Arc B-Series graphics are not supported with WSL2** — from an EOL-dated product's documentation, so it dates rather than settles the position. |
| `[GAP-DESKTOP26]` | The project's own hardware, read directly | Pilot desktop identity and PCIe topology, read from the machine 2026-08-14 via CIM and PnP device properties: Ryzen 7 7800X3D, 32 GB, MSI B650 GAMING PLUS WIFI (MS-7E26), Radeon RX 7900 XT. The card's link reports Gen4 x16 current and maximum — the card's own ceiling, since the 7900 XT is a PCIe 4.0 part, so this proves the slot is Gen4 x16 or better and cannot prove more; the root port exposes no link properties to Windows. Board slot layout is **search-level** (MSI's specification page returned 403): PCI_E1 PCIe 4.0 x16 from the CPU, PCI_E2 PCIe 3.0 x1 from the chipset, PCI_E3 PCIe 4.0 x4 — i.e. one graphics-usable slot. Confirm against MSI's page or the BIOS before anything rests on the second and third slots. PSU recorded 2026-08-14, stated by the creator from the unit: Corsair RM1000x (2021), 1000 W, 80+ Gold, fully modular. |
...
### Entry 077 — DSIT was abolished on 21 July 2026 and the AI skills function split three ways

- **Date logged:** 2026-08-12

- **Priority / Question:** Priority 1 — the department that owns every delivery figure in this log. Surfaced when WhatDoTheyKnow refused a request to DSIT, and checked directly rather than taken from that listing.

- **Source:** `[MOG-JUL26]`, plus the gov.uk departmental page carrying the replacement notice.

- **Checked date:** 2026-08-12

- **What the source directly supports:**

  - **DSIT no longer exists.** The written ministerial statement of 21 July 2026 states that "the functions of the Department for Science, Innovation and Technology (DSIT) will be redistributed", and that "where possible, these Machinery of Government changes will take practical effect from today." The gov.uk page for DSIT carries the notice: "This organisation is changing. It's being replaced by the Department for Business, Innovation, Science and Trade, the Department for Digital, Culture, Media and Sport and the Cabinet Office."

  - **The split, in the statement's own terms.** To the Cabinet Office: "responsibility for AI strategy, public sector AI adoption and the AI Security Institute", alongside a new AI Taskforce in a newly created Office for the Prime Minister and the Cabinet. To DCMS: "digital foundations, digital transformation, digital inclusion and skills, information resilience and online harms". To the Department for Business and Trade, renamed the Department for Business, Innovation, Science and Trade: "the science and innovation portfolio, as well as emerging sectors".

  - **AI Skills Boost is not named anywhere in the statement**, nor is the AI Skills Hub, nor the 10-million-worker target.

- **Inference drawn:** The department that set the 7.5-million-worker target in June 2025, raised it to 10 million in January 2026 and published the 1,001,147 course figure the same month was abolished six months later, and the function it sat in was divided between a central AI strategy team and a departmental skills brief. On the statement's wording, no successor is unambiguously the owner of a workforce AI-skills delivery programme: it is a skills programme, which points to DCMS, and an AI-strategy commitment, which points to the Cabinet Office. This project cannot say from the statement alone who now owns the target or its reporting — which is precisely the question the FOI now also tests.

- **Limitations / conflicting evidence:** The statement is the authoritative record of the change, not of where any individual programme landed; departmental allocation of specific programmes is usually settled in operational detail published later or not at all, and the ambiguity above is this project's reading of the wording, not a documented gap. Nothing here retracts any earlier entry: documents published by DSIT were published by DSIT, and remain the record of what was said at the time. Secondary coverage of the reshuffle was surfaced by search and has **not** been relied on; only the statement and the gov.uk notice are cited. Whether the 10-million target survives the reorganisation is unknown and must not be assumed either way.

- **Effect on project direction:** Request 1 in `drafts/foi_requests.md` re-addressed to DCMS, opening with the change, citing the statement, and invoking the section 45 transfer and section 16 advice duties so a wrong-door outcome is recoverable rather than fatal. Request 2 is unaffected: UKRI is a statutory body, and a machinery change moves its sponsorship without abolishing it. Every current-state reference to DSIT in project deliverables now needs checking before publication — the report's prose especially, where a present-tense "DSIT publishes" is now wrong. The Entry 064 fragmentation finding is strengthened: the delivery landscape has been reorganised again, mid-programme.
...
- **The hands-on break-even unit** — card decided 2026-08-13, the Arc
  Pro B70 at ~£1,290 for the 32 GB tier (`project_log.md` Entry 081).
  Run the llama.cpp and container stacks, measure tokens per second per
  pound per watt on defined SME-shaped tasks, publish including what
  failed. The draft document's closing section holds the design sketch.
...
### Entry 078 — Launch prices gathered: the memory crisis has moved Nvidia's flagship furthest from its own MSRP

- **Date logged:** 2026-08-12

- **Priority / Question:** Priority 6 — extending the Entry 068/070 pricing work at the creator's direction, so a published figure can show what each card was *meant* to cost alongside what it costs now. The street-only comparison could not distinguish a vendor's pricing strategy from the market's current distortion of it.

- **Source:** `[GPU-LAUNCH-PRICES26]` (added this date), with the RTX 5090 UK launch MSRP carried from `[GPU-PRICES-UK26]`.

- **Checked date:** 2026-08-12

- **What the sources directly support:**

  - **Consumer cards have vendor-published UK MSRPs; workstation cards do not.** Nvidia and AMD set sterling MSRPs for GeForce and Radeon consumer parts (RTX 5070 £539, RTX 5060 Ti 16GB £399, RTX 5090 £1,919, RX 9060 XT 16GB £314.99), and Intel did for the consumer Arc B580 (£249.99). No UK RRP was found for any workstation card — Intel's Arc Pro B50/B60/B70 or AMD's Radeon AI PRO R9700 — which are partner-sold and quoted in US dollars.

  - **Intel never published an MSRP for the Arc Pro B60 at all.** It indicated ~$500 at Computex 2025; the launch retail entry was $599 (ASRock) and Sparkle listed $799. The card most central to the 24 GB comparison therefore has no vendor list price, only a retail range.

  - **The Arc Pro B50 launched above its announced price**, $349 against a $299 Computex target, reported as memory-cost and tariff driven — the memory crisis reaching a card before it shipped.

  - **Measured against their own launch prices, the cards have diverged sharply.** RTX 5090 £1,919 to £4,199 (2.2x); Arc Pro B60 ~£532 converted to £830 (1.6x); Arc Pro B70 ~£842 converted to £1,290 (1.5x); RTX 5060 Ti £399 to £450 (1.13x); RTX 5070 £539 to £599 (1.11x); R9700 ~£1,153 converted to £1,250 (1.08x); RX 9060 XT £315 to £330 (1.05x); Arc B580 £250 to £245 (0.98x, the only card below its launch price).

- **Inference drawn:** The premise that UK street pricing understates Intel's case is half right, and the half that is wrong matters more. Intel's *workstation* cards do carry the widest UK premium of any new card here (1.5–1.6x list), so at 24 GB the street price is what erases the B60's advantage over a used 3090 — that caveat is real and survives. But at the headline comparison the direction reverses: because the RTX 5090 has moved 2.2x from its own UK MSRP, the Intel-versus-Nvidia gap at 32 GB is *wider* at street prices (3.3x) than at launch prices (2.3x). Street pricing flatters Intel at the top of the table and penalises it in the middle. Both readings are now visible in one figure rather than one being asserted.

- **Limitations / conflicting evidence:** The two price bases are not equivalent and the figure marks which is which: a vendor UK MSRP is a sterling price inclusive of VAT, whereas the workstation figures are US list converted at one day's rate with VAT added, which assumes a UK price would be set by conversion — it demonstrably is not, since the observed UK premium is exactly what the chart shows. Launch dates span December 2024 to March 2026, so "launch price" is not one moment in the memory market. The RX 9060 XT and R9700 figures are search-level, both direct fetch routes having failed for the former. The RTX 3090 has no comparable launch price and is shown used-only. MSRP is a stated intention rather than a transaction, and several of these cards never sold at theirs.

- **Effect on project direction:** Rebuilds `assets/figures/vram_price_ladder` as a launch-to-street comparison — `project_log.md` Entry 076. `drafts/budget_vram_for_local_ai.md` gains a launch-price column and the reversal above as a finding; its "the B60's UK position is weaker than its design" bullet is corroborated with a number rather than an inference.

### Entry 079 — What fits in 32 GB measures like: today's single-card models sit where the frontier stood in late 2024

- **Date logged:** 2026-08-12

- **Priority / Question:** Priority 6 — the capability side of the local-vs-cloud question. Entries 030 and 042 established the open-vs-frontier lag in general terms; this entry answers the narrower question a 32 GB buyer actually faces — what do the models that *fit* measure like, against today's frontier and against the past frontier products a lay reader has used — prompted by the creator's direction to translate the comparison for a public audience with era anchors rather than an index number.

- **Source:** `[EPOCH-ECIDATA26]` (primary scale), `[EPOCH-GPUGAP25]`, `[OPENAI-4OMINI24]`, `[AA-INDEX26]` (cross-check), `[LLMEXPLORER26]`; capability-gap context carried from `[EPOCH-ECIGAP26]` (Entry 042).

- **Checked date:** 2026-08-12

- **What the sources directly support:**

  - **The "open models are ~4 months behind" headline does not describe single-card models.** Epoch's four-month gap (`[EPOCH-ECIGAP26]`) is earned by frontier open-weight releases: Kimi K3 (ECI 157.0) is a 2.8-trillion-parameter mixture-of-experts model, Kimi K2.6 (151.1) one trillion, Inkling (149.5) 975 billion (parameter counts search-level: Artificial Analysis' own release article, TechCrunch, vendor model cards). Mixture-of-experts thrift is compute per token, not memory — every parameter must sit in memory to be reachable — so these run on server racks. Three tiers, not two: closed frontier, open frontier (rack-scale), single-card.

  - **The best single-card models on the ECI scale (retrieved 2026-08-12):** Qwen3.6 35B-A3B at 143.5 [95% CI 137.8–148.1] and Gemma 4 31B IT at 142.3 [134.5–146.9], both released April 2026, both under the ~40B-parameter ceiling Epoch's own consumer-GPU memory model puts on a ~30 GiB card at 4-bit (`[EPOCH-GPUGAP25]`). At 4-bit both also load on a 24 GB card (~17–20 GB of weights), with 32 GB buying context headroom. Nothing scored above them fits. Two gaps in the scale itself: Qwen3.6 27B dense and gpt-oss-20b carry no ECI (insufficient benchmark coverage in the dataset); Artificial Analysis scores them 38 (its small-open leader) and 15 respectively. Gemma's licence is tagged "restricted use" in Epoch's data, unlike the Apache Qwen.

  - **The frontier and the era anchors, same scale:** GPT-5.6 Sol 161.7, Claude Fable 5 161.5, Claude Opus 5 161.0 (mid-2026 frontier); GPT-5 150.0 (Aug 2025; a calibration anchor, no CI); o3 146.9 (announced 2024-12-20, released Apr 2025); Gemini 2.5 Pro 144.5 (Mar 2025); o1 142.5 [140.3–143.4] (Dec 2024); GPT-4o 128.6 (May 2024); GPT-4o mini 126.6 (Jul 2024); GPT-4 125.7 (Mar 2023); GPT-3.5 Turbo 112.5–117.8 by revision.

  - **The lag, computed rather than asserted:** the closed frontier first met the current single-card best (143.5) with o3 — announced December 2024, publicly released April 2025 — with Gemini 2.5 Pro (144.5) publicly available from late March 2025. So the best a card runs in August 2026 sits where the released frontier stood roughly 16–20 months earlier; "about a year and a half" is the honest public phrasing. Longer than `[EPOCH-GPUGAP25]`'s 6–12 months measured a year earlier — directionally consistent with Epoch's own finding that the open-closed gap widened from three to four months over 2025.

  - **Which era-anchor comparisons survive their confidence intervals:** both single-card models' CI lower bounds (137.8, 134.5) clear every 2023–24 ChatGPT-era model's upper bound (the highest, GPT-4o's Nov 2024 revision, tops out at 131.3), so "clearly above every ChatGPT model of 2023–24" holds outright. Against o1 (142.5) the intervals overlap — "at the level of", never "ahead of". Product history for the labels: GPT-4 was ChatGPT's paid tier and was never the free model; free ChatGPT ran GPT-3.5 until GPT-4o mini replaced it on 2024-07-18, "in place of GPT-3.5" (`[OPENAI-4OMINI24]`, exact words).

  - **Cross-check:** Artificial Analysis' independently-constructed composite gives the same ordering on its 2026-08-12 snapshot — frontier 61–63, Kimi K3 60, Qwen3.6 27B 38 leading the ≤40B open class (`[AA-INDEX26]`). Unsuitable as the published scale (rescaled between versions; reproduction terms unclear, where Epoch is CC-BY), but the agreement means the finding does not rest on one index's construction.

  - **The lead that started the thread mismeasures the question:** llm-explorer's 32 GB list assumes 16-bit weights, so it excludes everything above ~16B parameters — exactly the 27–36B class the capacity exists to buy — and its popularity-weighted default sort surfaces 2024-era models first (`[LLMEXPLORER26]`).

- **Inference drawn:** For the public figure: a 24–32 GB card in August 2026 runs models at the level the paid frontier held around the turn of 2025 — clearly above everything ChatGPT offered through 2024, level with the first reasoning models, about a year and a half behind today's frontier. An index point means nothing to the intended reader, so the figure's job is anchoring the scale to dated products the reader has used and letting time carry the meaning; the number itself stays de-emphasised.

- **Limitations / conflicting evidence:** ECI confidence intervals on the small models are wide (roughly ±5) and the scale's two calibration anchors are pinned by construction. A composite compresses task-specific differences — Entry 042's finding that the gap concentrates in reasoning and agentic work is invisible in a single number — and Epoch warns small models are likelier tuned to benchmarks, which would flatter the single-card tier; both push the same way, so the figure's claim is stated as a floor-to-level, not a ceiling. "Fits" uses Epoch's memory model (4-bit, 8K context): long context or higher precision shrinks what fits, and the 12 GB tier (the £245 B580) runs none of the models scored here. Parameter counts for the rack-scale open models are search-level. Task-level adequacy for SME workloads remains untested here — this entry anchors the general level, nothing more.

- **Effect on project direction:** `tools/build_vram_figures.py` gains a third figure, `assets/figures/vram_capability_ladder` (light/dark, SVG/PNG) — production decisions in `project_log.md` Entry 078. `drafts/budget_vram_for_local_ai.md`'s capability CHECK callout is rewritten on this entry (three tiers, era anchors, the figure) and its sources line extended. The hands-on unit's task-level question is unchanged — and now has a stated baseline to test against.
### Entry 080 — The Lifelong Learning Entitlement funds modules, but not the kind of module GAP is building

- **Date logged:** 2026-08-13

- **Priority \ Question:** Priority 10 (delivery format and validation) — is there a public funding route for a short, standalone AI capability unit?

- **Source:** `[LLE-GOV26]`

- **Checked date:** 2026-08-13

- **What the source directly supports:** The Lifelong Learning Entitlement replaces undergraduate student finance and Advanced Learner Loans at Levels 4, 5 and 6 with a single credit-based post-18 system in England. Applications open September 2026 for courses and modules starting from January 2027. Funding is denominated in credits rather than years: 120 credits is a full-time year, a learner may draw between 30 and 180 credits in any 12-month period, and the total entitlement is 480 credits, stated as £39,160 for courses starting after 1 January 2027. Individual modules carry 15 to 60 credits. Module-level funding is **not** open to all provision: it covers modules of high-value technical courses at Levels 4 and 5, and modules from full Level 6 qualifications, in both cases where they align to priority skills needs and the industrial strategy.

- **Inference (the project's own, not stated by the source):** The lead reached the project as "government funding at module and learning-credit level, build awareness of it in", which reads as an opportunity. Read against the eligibility rule it is closer to a constraint. GAP's pilot unit is 45–60 minutes, non-accredited, and attached to no Level 4–6 qualification; the smallest fundable unit here is a 15-credit module inside an accredited technical course, which is roughly two orders of magnitude larger. The LLE therefore does not reach the pilot as scoped, and would only become relevant if the project pursued accreditation and alignment to a named priority skill — a different project shape, not a funding tweak.

- **Contradicting evidence / caveats:** **Search-level only.** Neither the gov.uk provider guidance nor the Commons Library briefing was fetched directly, and every figure above needs a primary read before it is used anywhere outward-facing. The £39,160 figure and the 30/180-credit bounds are the ones most likely to be misreported at search level. Whether a non-accredited unit could ever be brought inside an eligible module, rather than standing alone, is not addressed by the source and is not knowable from it.

- **Effect on project direction:** Closes a funding route rather than opening one, which is the more useful outcome. Recorded so the LLE is not carried forward as an assumed option. Does not change the pilot's scope, which was never predicated on public funding.

### Entry 081 — Government is launching short AI training units of its own, on the timetable GAP is building into

- **Date logged:** 2026-08-13

- **Priority \ Question:** Priority 10, and Priority 2 (SME access) — does an existing public route already deliver short AI capability training to SME staff?

- **Source:** `[APPRENTICE-AUG26]`

- **Checked date:** 2026-08-13

- **What the source directly supports:** From August 2026 the 5% co-investment requirement for non-levy-paying employers is removed for apprentices under 25, making that training fully government funded, alongside employer incentive payments reported as a £2,000 SME hiring bonus and stackable grants, and National Insurance relief. Separately and more significantly for this project, the reforms include **"apprenticeship units" — short, flexible training courses in AI, digital and engineering, reported as launching April 2026 with five to ten initially.** Levy-payer terms tighten in the same package: co-investment rises from 5% to 25% once levy funds are exhausted, and unused levy funds now expire after 12 months rather than 24.

- **Inference (the project's own, not stated by the source):** Short, flexible, government-funded AI training units aimed at employers are the same shape as GAP's pilot unit and reach the same population the project has identified as underserved. That is not automatically competition — the project's whole critique is that publicly delivered AI training is counted in course completions rather than capability — but it is the first public provision found that matches GAP's own format rather than its subject. It needs a primary read and, if real, a direct look at the content, in the same way the AI Skills Hub was examined first-hand.

- **Contradicting evidence / caveats:** **The weakest-sourced entry in this log, and it is logged as a lead rather than a finding.** Nothing was fetched from gov.uk; every reachable source was a training provider or sector body with a commercial interest in apprenticeship uptake, which is precisely the interest-type this project deprioritises. The "5-10 initially" figure and the April 2026 launch are single-source and uncorroborated. It is also unresolved whether these units sit only inside apprenticeship standards or are available standalone — which decides whether they reach SME staff who are not apprentices, and therefore whether they touch GAP's audience at all.

- **Effect on project direction:** Opens a thread rather than settling one. Two things now need a primary pass: the gov.uk apprenticeship funding rules for August 2026 onward, and whatever exists of the apprenticeship units themselves. The original question that produced this lead — where practical AI skills sit within digital and IT apprenticeship standards — remains unanswered and is carried to Open Threads.
### Entry 082 — Intel ships a telemetry tool that reports Arc Pro power in watts, correcting Entry 071's conclusion

- **Date logged:** 2026-08-13

- **Priority / Question:** Priority 6 — the idle-power hole in the budget-VRAM document, open since Entry 071 and recorded there as unreachable in software.

- **Source:** `[INTEL-XPUM26]` (added this date). Surfaced as a bare pointer during an external technical exchange, then read directly at the vendor's own repository. The exchange is not evidence and is not cited; the finding rests entirely on Intel's published documentation.

- **Checked date:** 2026-08-13

- **What the source directly supports:** Intel publishes XPU Manager, a free and open-source GPU monitoring and management tool, with `xpu-smi` as its local command-line interface. Its supported-devices list names the **Intel Arc Pro Series GPU**. It runs on Linux, with Ubuntu 24.04 and 26.04 listed, and on Windows Server with limited features. Among the metrics it exposes is **power usage against the card's cap, in watts**, alongside GPU utilisation, temperature, frequency, memory usage and throughput, fabric speed and port health, and volatile uncorrectable ECC error counts.

- **Inference (the project's own, not stated by the source):** Entry 071 recorded that the current Linux driver does not expose GPU power in software, and concluded from it that settling idle power requires a wall meter on real hardware. The first half may still hold of the kernel driver's sysfs interface — XPU Manager is a userspace tool and most likely reads through level-zero sysman rather than sysfs — but **the conclusion drawn from it was wrong**: GPU-level power on an Arc Pro card is reachable in software on Linux. The wall meter is not thereby redundant. It measures what the whole machine draws at the plug, including PSU losses, CPU and everything else, which is the figure a running-cost argument actually needs. The two instruments answer different questions and the document should carry both rather than swapping one for the other.

- **Contradicting evidence / caveats:** Read from the project's own repository documentation only. **Nothing is verified on hardware** — this project has run no Arc card. Whether a B70 specifically enumerates, and whether the idle figure it reports is meaningful rather than a floor or a cap-derived estimate, are untested. The tool is presented as a data-centre product with Arc Pro appearing in its device list, so coverage may be uneven across the workstation line. First-party vendor tooling is also an interested source on its own hardware's behaviour, and the interest runs toward flattering idle numbers.

- **Effect on project direction:** `drafts/budget_vram_for_local_ai.md` carries a statement in "What this document does not count" that is now wrong on the mechanism, and it must be corrected — GPU power is obtainable in software; what is missing is the measurement, not the possibility. The Open Threads idle-power item changes from "settling it takes a wall meter" to "it takes a card, `xpu-smi` for the GPU figure, and a wall meter for the system figure". Adds a concrete, cheap first step to the pilot unit's Arc phase, and the same tool's ECC counters give the Entry 083 ECC thread an empirical route rather than a specification-sheet one.
### Entry 083 — Three gaps in the budget-VRAM document, all about operating the hardware rather than buying it

- **Date logged:** 2026-08-13

- **Priority / Question:** Priority 6 — three claims raised during an external technical exchange that the budget-VRAM document does not address, each checked independently afterwards.

- **Source:** `[ARCPRO-ECC26]`, `[GB10-SPECS26]`, `[KATAGO-BACKENDS26]` (all added this date). The exchange is not evidence and is not cited; each finding rests on published material located afterwards. Companion to Entry 082, which came from the same exchange and is the one item of the four that reached a primary source.

- **Checked date:** 2026-08-13

- **What the sources directly support:**

  - **The Arc Pro B70 carries ECC memory.** 32 GB of ECC GDDR6 on a 256-bit bus at 608 GB/s, reported consistently across launch coverage, reviews and retail listings, alongside ISV-certified drivers (`[ARCPRO-ECC26]`). No consumer card in the document's comparison table has ECC — not the used RTX 3090, not the RTX 5090, not the Arc B580.

  - **Unified-memory machines trade bandwidth for capacity, and the trade is large.** The HP ZGX Nano G1n runs 128 GB of LPDDR5X unified memory at up to 273 GB/s (`[GB10-SPECS26]`) against the B70's 608 GB/s — roughly 2.2x in the discrete card's favour, at a quarter of the capacity.

  - **OpenCL is a working vendor-neutral path on Intel hardware.** KataGo ships an OpenCL backend documented as running on any OpenCL-capable GPU including Intel, with no CUDA-style install, and self-tuning per GPU on first run (`[KATAGO-BACKENDS26]`). The project's own documentation warns that OpenCL implementation quality is inconsistent.

- **Inference (the project's own, not stated by the sources):** `drafts/budget_vram_for_local_ai.md` misses all three, and they fail in the same direction. **Every one concerns what the hardware is like to own and operate rather than what it costs to buy.** ECC is a data-integrity property that matters for long runs and regulated work, and it separates the B70 from every consumer card in the table on something other than price. The bandwidth trade qualifies the document's line deferring unified-memory machines as "the other genuine route" — they are a genuine route to *capacity*, at a substantial cost in speed the document never states. And the software section's four paths (Vulkan, SYCL, the archived IPEX-LLM, LLM Scaler/vLLM) omit the one that needs no vendor blessing at all, which bears directly on the roadmap risk the same document worries about. The document was built to answer a purchase comparison — price per gigabyte, tokens per second — and it answered that well while leaving the operating envelope unexamined.
...
### Entry 086 — The Arc Pro software stack read from the vendor: OMIX is real, and the B70 and A770 sit on different driver generations

- **Date logged:** 2026-08-14

- **Priority / Question:** Priority 6 — the software-stack risk that decides whether the budget-VRAM findings are operable (Entries 069, 073), now concrete because the project's own B70 is purchased (`project_log.md` Entry 081) and its host environment has to be chosen.

- **Source:** `[INTEL-DGPU26]`, `[IPEX-EOL26]`, `[PYTORCH-XPU26]`, `[OLLAMA-DOCS26]` (all added this date). Prompted by two source-checked baseline documents received during the external technical exchange (`project_log.md` Entry 082) — the name "OMIX" itself first entered the exchange as an unverified AI-search result, correctly flagged as such at the time. The exchange and the documents are not evidence and are not cited; every claim below rests on vendor pages read directly this date.

- **Checked date:** 2026-08-14

- **What the sources directly support:**

  - **Intel Open Middleware Xe (OMIX) exists and is the vendor's validated stack for the Arc Pro B-series.** Intel's developer documentation carries an OMIX installation guide and support matrix: a pinned, validated GPU software stack (Level Zero/OpenCL drivers, SYCL compiler, oneDNN/oneMKL, the runtime components PyTorch workloads need) for Arc Pro B50/B60/B65/B70, installable on Ubuntu Desktop 26.04, 24.04.4, or 24.04 with the 6.17 HWE kernel. The matrix lists three releases already (0.1.0/0.2.0/0.3.0), the B70 validated in all three, with Ubuntu 26.04 the listed host OS throughout (`[INTEL-DGPU26]`).

  - **The A770 and the B70 sit on different kernel driver generations.** Intel's Xe KMD hardware table carries the B70 as device E223 (Xe2/Battlemage) with full support at Ubuntu 25.10+ / kernel 6.17 — and does not carry the A770 at all, which as an Alchemist part remains on the older i915 driver. Driver-level workarounds therefore do not transfer between the two cards in either direction (`[INTEL-DGPU26]`).

  - **The second consumer-path retirement now has a date.** Intel Extension for PyTorch "will reach end of life (EOL) by the end of March 2026", with Intel recommending native PyTorch, into which its CPU and GPU support has been upstreamed (`[IPEX-EOL26]`). This is distinct from the IPEX-LLM archival of January 2026 already logged (Entry 069, `[IPEXLLM-GH26]`) — two different projects, both ended within a quarter. PyTorch's own client-GPU page lists Arc A-series and B-series validated on Windows 11 and Ubuntu 24.04/25.10, with Ubuntu 26.04 not yet in the validated list (`[PYTORCH-XPU26]`).

  - **Intel's B70-validated containers exist and carry their own warnings.** The PyTorch 2.11.0 and vLLM 0.21.0 container release notes both list the B70, both validated on Ubuntu 24.04.4 hosts, both OMIX-based, and both state "not intended for production use". The PyTorch notes add that the bundled oneCCL "does not include full optimization for PCIe collective communications", so multi-GPU use will not reach full performance; the vLLM notes flag a dependency CVE (`[INTEL-DGPU26]`).

  - **Ollama's Vulkan path is now default-on, not opt-in.** Vulkan is bundled in the official Docker image and "enabled by default when the container can access the GPU devices"; `OLLAMA_VULKAN=0` disables it and `GGML_VK_VISIBLE_DEVICES` selects devices. Neither page fetched carried an "Experimental" label at reading (`[OLLAMA-DOCS26]`).

  - **The client install route for both Ubuntu LTS releases is one PPA.** Intel's client guide uses `ppa:kobuk-team/intel-graphics` for Ubuntu 26.04 and 24.04, and states "Ubuntu 26.04 and later provides native support for Lunar Lake, Battlemage, and Panther Lake" (`[INTEL-DGPU26]`).

- **Inference (the project's own, not stated by the sources):** The software-risk picture in `drafts/budget_vram_for_local_ai.md` moves in both directions at once. Both individual-consumer paths are now formally dead on Intel's own pages — IPEX-LLM archived in January, IPEX proper EOL in March — which hardens the document's warning. But a vendor-validated workstation path has arrived that the document does not know about: OMIX is native packages on a desktop OS for exactly the Arc Pro line the project has bought into, at three releases in the five months since the B70 launched, which softens the "containers-only, enterprise-shaped" reading without dissolving it — OMIX is still operator-shaped, pinned stacks on clean systems. The kernel-floor figure in the same draft ("6.12+", from `[BENTECH-ARC26]`) needs a per-card restatement: it described the April 2026 stack as one practitioner ran it, and Intel's own floor for the B70's driver generation is kernel 6.17. For the project's own machines, an existing Ubuntu 26.04 install sits on the right side of every support line above without modification. *(Applicability corrected 2026-08-14, same day: this sentence was written expecting the always-on server to host the card. The host is the desktop — `project_log.md` Entry 083 — which does not currently run Ubuntu at all. The Intel facts above are unaffected; what changes is which machine has to be brought to them, which Entry 087 works through.)*

- **Contradicting evidence / caveats:** Every source in this entry is first-party — Intel on Intel's stack, Ollama on Ollama's — and no independent coverage of OMIX was read this pass; a five-month-old 0.x stack with three releases is a cadence, not a maturity finding. The documentation is visibly mid-transition and internally inconsistent — the containers are validated on Ubuntu 24.04.4 hosts while the OMIX matrix lists 26.04 as the host OS — so the correct action at install time is to freeze against the then-current validated matrix, not against this entry. Two retrieval holes, named per the bias checklist: intel.com's B70 specification page returned 403 to the fetcher (blocked, not absent — its figures are corroborated by `[ARCPRO-ECC26]`-tier coverage and board-partner listings, and Entry 083's requirement of a primary pass on ECC stands), and the dgpu-docs landing page returned an empty body, so the deep pages were read without their index.

- **Effect on project direction:** Two further corrections queue for `drafts/budget_vram_for_local_ai.md`, joining Entry 083's three: OMIX added as a software path, and the Ollama line refined from "no native Arc support" to "no Arc-specific backend; the generic Vulkan path is now default-on". The pilot's Arc phase inherits a concrete bring-up baseline — Ubuntu 26.04, a kernel 6.17-class Xe KMD, then OMIX or the bare compute runtime, verified at each stage (device E223 in lspci, the device name in clinfo, real tensor work under torch.xpu). The host-machine question this raises is a hardware decision, not a research one, and is recorded in `project_log.md` Entry 082.

### Entry 087 — What the desktop host costs the Arc plan: no WSL2 path, one graphics slot, and no need for an Intel platform

- **Date logged:** 2026-08-14

- **Priority / Question:** Priority 6 — what the creator's decision to host the B70 in the existing Windows desktop (`project_log.md` Entry 083) requires and forecloses, checked before any hardware is bought or reinstalled.

- **Source:** `[PYTORCH-XPU26]` and `[INTEL-DGPU26]` (both read directly, Entry 086), `[ARC-PLATFORM26]` (search level), `[GAP-DESKTOP26]` (the project's own machine, read directly).

- **Checked date:** 2026-08-14

- **What the sources directly support:**

  - **No vendor documentation places a B-series Arc card under WSL2.** PyTorch's Intel-GPU page names Windows 11 and Ubuntu 24.04/25.10 as the validated client-GPU operating systems and does not mention WSL2 anywhere (`[PYTORCH-XPU26]`). Intel's own IPEX documentation goes further and states that WSL2 support is experimental and that Arc B-Series graphics are **not** supported with WSL2 — from a product Intel has dated to end-of-life in March 2026, so it dates the position rather than settling it (`[ARC-PLATFORM26]`).

  - **Native Windows is a validated route, but only for part of the stack.** PyTorch XPU lists Windows 11 for Arc A- and B-series (`[PYTORCH-XPU26]`), and Ollama's Vulkan backend covers Windows (`[OLLAMA-DOCS26]`, Entry 086). Everything else Intel validates for the B70 is Linux: OMIX ships as Ubuntu packages, and the B70-validated PyTorch and vLLM containers are Linux containers on Ubuntu 24.04.4 hosts (`[INTEL-DGPU26]`).

  - **Resizable BAR is the platform requirement, and it is not an Intel-CPU requirement.** ReBAR is stated as required for intended performance on Arc Pro A- and B-series, with Above 4G Decoding its BIOS precondition. Intel's named platform list is Intel-centric, but the documentation states that other platforms with ReBAR or AMD Smart Access Memory enabled may also support these cards, and Ryzen 7000 appears in the surfaced coverage (`[ARC-PLATFORM26]`).

  - **The desktop's board gives one graphics-usable slot, at Gen4.** Read from the machine: Ryzen 7 7800X3D, 32 GB, MSI B650 GAMING PLUS WIFI, Radeon RX 7900 XT negotiating Gen4 x16 — which is the 7900 XT's own ceiling as a PCIe 4.0 part, so it proves the slot is Gen4 x16 *or better* and cannot prove more. The board's published layout is PCI_E1 at PCIe 4.0 x16 from the CPU, PCI_E2 at PCIe 3.0 x1 from the chipset, PCI_E3 at PCIe 4.0 x4 (`[GAP-DESKTOP26]`, slot layout search-level).

- **Inference (the project's own, not stated by the sources):** Three consequences, in descending order of how much they change the plan.

  **The deployment-shape phase cannot use WSL2 for the Intel card.** `drafts/pilot_ai_workstation.md` Phase 2 is built on WSL2 because AMD's compatibility matrix supports the 7900 XT there. Nothing equivalent exists for a B-series Arc card, and the one Intel statement found excludes it. So the containerised phase — which is the phase that proves the *product* shape, not just inference — means native Linux on the desktop for the Arc half. That is also exactly what the shared environment agreement with the external correspondent assumes (`project_log.md` Entry 082), so the two requirements coincide rather than compete.

  **On this board the two cards cannot both be GPUs.** PCI_E2 at Gen3 x1 is not a graphics slot in any useful sense, so the 7900 XT and the B70 take turns in PCI_E1. That forces sequencing rather than a choice: finish the AMD phases 1–2 under Windows and WSL2 as written, then swap cards for the Arc work. It also means the multi-card configuration discussed with the correspondent — two B70s at Gen5 x8+x8 — is impossible on this board, which is a board limit, not a CPU-vendor one.

  **The floated Intel motherboard and CPU purchase is not required by the card.** ReBAR is the requirement and an AM5 Ryzen 7000 platform satisfies it. What a board change would actually buy is slot topology — a second CPU-fed x16 slot for card coexistence or multi-GPU, and Gen5 — and neither is needed for single-card inference, where the weights sit in VRAM and the link carries loading rather than generation. The B70 document's own §8 makes the same point before requiring the link be recorded anyway. So the purchase question is real but should be argued on slots, not on vendor matching, and it is not on the critical path to first tokens.

- **Contradicting evidence / caveats:** The two load-bearing negatives are the weakest-sourced parts. The WSL2 exclusion comes from an EOL-dated product's documentation at search level, and PyTorch's silence is an absence of validation rather than a statement of impossibility — the honest position is "undocumented and once explicitly excluded", not "proven not to work", and it is cheap to test once the card is here. The board slot layout is search-level because MSI's specification page returned 403; the machine could confirm Gen4 x16 on PCI_E1 but not the other two slots, and the root port exposes no link properties to Windows. Intel's own quick-start article also returned 403, so the ReBAR and AMD-platform statements are search-level. PSU capacity is not readable in software and remains unrecorded on this machine — with a 230 W-TBP card joining a ~315 W-class one in a box where they may briefly both be installed, that is the check with the most physical risk attached.

- **Effect on project direction:** `drafts/pilot_ai_workstation.md` is revised in the same session: the machine table gains the board and the unrecorded PSU, Phase 2 is marked AMD-only, and the Arc section becomes a host-and-OS decision with the three routes priced. Three checks queue before the card arrives — MSI's own slot specification read by hand, the PSU rating read off the unit, and ReBAR/Above 4G confirmed in BIOS. Whether the desktop ends up dual-booting or running Ubuntu outright is the open question the Arc phase now turns on.
